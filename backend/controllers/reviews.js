...
exports.updateReview = (req, res) => {
  ...
  const { rating, review_text, replaceAllImages } = req.body;
  const userId = req.user ? req.user.id : null;
  const newImages = req.files; // Multer will populate this with new images
  ...
  console.log('Replace all images:', replaceAllImages);
  console.log('New images:', newImages ? newImages.length : 0);
  console.log('Files received:', newImages);
  ...
  connection.query(verifySql, [review_id, userId], (err, results) => {
    ...
    connection.beginTransaction(err => {
      ...
      connection.query(updateSql, [rating, review_text, review_id], (err, result) => {
        ...
        console.log('Review text/rating updated successfully');
      ...
      function handleImageUpdates() {
        ...
        console.log('New images available:', newImages ? newImages.length : 0);
        console.log('Replace all images flag:', replaceAllImages);


        if (newImages && newImages.length > 0) {
          if (replaceAllImages === 'true') {
            console.log('Replacing all existing images');
            // Remove all existing images first
            const deleteAllSql = 'DELETE FROM review_images WHERE review_id = ?';
            connection.query(deleteAllSql, [review_id], (err, deleteResult) => {
              if (err) {
                return connection.rollback(() => {
                  console.error('Error deleting existing images:', err);
                  res.status(500).json({ success: false, message: 'Failed to remove existing images' });
                });
              }
              console.log('Deleted existing images:', deleteResult.affectedRows);
              insertNewImages();
            });
          } else {
            console.log('Adding new images without removing existing ones');
            insertNewImages();
          }
        } else {
          console.log('No new images to process, committing changes');
          commitChanges();
        }
      }
      ...
      function insertNewImages() {
        ...
        console.log('Images to insert:', newImages.length);
        ...
        newImages.forEach((file, index) => {
          ...
          imageData.push([review_id, file.filename]);
        });


        console.log('Image data prepared for insertion:', imageData);


        if (imageData.length === 0) {
          console.log('No valid image data to insert');
          return commitChanges();
        }


        // Create the SQL with proper placeholders
        const placeholders = imageData.map(() => '(?, ?, NOW())').join(', ');
        const insertSql = `INSERT INTO review_images (review_id, image_path, created_at) VALUES ${placeholders}`;
        const flatData = imageData.flat();


        console.log('Insert SQL:', insertSql);
        console.log('Insert data:', flatData);


        connection.query(insertSql, flatData, (err, insertResult) => {
          if (err) {
            return connection.rollback(() => {
              console.error('Error inserting images:', err);
              res.status(500).json({ success: false, message: 'Failed to save images' });
            });
          }


          console.log('Images inserted successfully:', insertResult.affectedRows);
          commitChanges();
        });
      }
      ...
      function commitChanges() {
        ...
        const getImagesSql = 'SELECT image_path FROM review_images WHERE review_id = ? ORDER BY created_at ASC';
        connection.query(getImagesSql, [review_id], (err, imageResults) => {
          if (err) {
            return connection.rollback(() => {
              console.error('Error fetching final images:', err);
              res.status(500).json({ success: false, message: 'Failed to fetch images' });
            });
          }


          const finalImages = imageResults.map(img => img.image_path);
