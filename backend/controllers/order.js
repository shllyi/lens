    ...
    `;
      ...
      db.query(orderlineSql, [orderlines], (err) => {
        ...
        db.commit(err => {
          ...
          // Send order confirmation email with receipt (don't wait for email to send)
          sendOrderConfirmationEmail(orderinfo_id, customer_id, items).catch(console.error);
          ...
          res.json({ 
            ...
            orderinfo_id,
            customer_id // Include customer_id in the response
          });

          // Then force a small delay to ensure the transaction is fully committed
          // before the client makes the next request
          setTimeout(() => {
            // This is just to ensure the transaction is fully committed
            // No action needed, just a small delay
          }, 100);
        });
      });
    });
...
const sendOrderConfirmationEmail = async (orderinfo_id, customer_id, items) => {
  try {
    console.log(`Attempting to send order confirmation email for order #${orderinfo_id}`);
    ...
    db.query(getOrderDataSql, [orderinfo_id], async (err, results) => {
      if (err) {
        console.error('Database error when fetching order data:', err);
        return;
      }
      
      if (results.length === 0) {
        console.error(`No order found with ID: ${orderinfo_id}`);
        return;
      }
      
      if (!results[0].email) {
        console.error(`No email found for order #${orderinfo_id}`);
        return;
      }
      ...
      db.query(getItemsSql, [orderinfo_id], async (err, orderItems) => {
        if (err) {
          console.error('Error fetching order items:', err);
          orderItems = []; // Continue with empty items
        }
